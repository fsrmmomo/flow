...
//            gettimeofday(&start, NULL );
            ...
            printf("len = %d\n",length);
            VLOG_INFO("\nkey_num = %d\n",length);
            uint8_t *buf = (uint8_t*)malloc(length+4);
...
void *tcp_server(void *arg){
    ...
    local_addr.sin_addr.s_addr = inet_addr("192.168.1.81");	//　设置IP地址
    int ret = bind(slisten, (const struct sockaddr *)&local_addr, sizeof(local_addr));
    if (ret < 0)
        perror("bind");
    else{
        VLOG_INFO("bind success.\n");
        // printf("bind success.\n");
    }
...
pmd_thread_main(void *f_)
{
    ...
    /**************my measurement start***********/
    //开启接收线程
// #ifdef SIGCOMM_MEASUREMENT 
//     pthread_t thread_id;
//     VLOG_INFO("\naddr:%u\n",&(pmd->big_key));
//     pthread_create(&thread_id, NULL, tcp_server,&(pmd->big_key));
// #endif
    /**************my measurement end***********/
        ...
        struct dp_netdev_flow *flow;
         ...
         if(likely(mbuf != NULL)){
             ...
             if(eth_hdr->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)){
                 ...
                 printf("%u\n\n",(uint32_t)ip_hdr->src_addr);
                 VLOG_INFO("\na key:%u\n",get_a_key(&(pmd->big_key)));
                 VLOG_INFO("\naddr:%u\n",&(pmd->big_key));
                 VLOG_INFO("\npmdaddr:%u\n",pmd);
                 VLOG_INFO("\nPMD->THREADID:%u\n", pmd->thread);
                 VLOG_INFO("\ncur_ip:%u  %d %d\n\n",ip_hdr->src_addr,pmd->big_key.Check(&(pmd->big_key),(uint32_t)ip_hdr->src_addr),pmd->big_key.w);
                 if(pmd->big_key.Check(&(pmd->big_key),(uint32_t)ip_hdr->src_addr)){
                    VLOG_INFO("\nbchange tos:%d\n",ip_hdr->type_of_service);
                    ip_hdr->type_of_service |= 1;
                    VLOG_INFO("\nachange tos:%d\n",ip_hdr->type_of_service); 
                    // packet_set_ipv4(packet,ip_hdr->src_addr,ip_hdr->dst_addr,ip_hdr->type_of_service,ip_hdr->time_to_live);
                    VLOG_INFO("\nchange tos:\n");
                 }
