...
def train_and_predict():
    ...
    debug("# AR test instances {}".format(len(AR) - len(AR_train)))
    ...
    for i in train_y:
        if i == 0:
            train_y_name.append("VIDEO")
        if i == 1:
            train_y_name.append("IOT")
        if i == 2:
            train_y_name.append("车载网")
        if i == 3:
            train_y_name.append("AR/VR")
    ...
    # # print(dataframe)
    # dataframe.to_excel('./data_train.xls', index=False)
    # info('打印训练集')
    ...
    print("#AR test {}".format(len(AR_test)))
    print("##########################")
    ...
    for i in test_y:
        ...
        if i == 2:
            test_y_name.append("车载网")
        if i == 3:
            test_y_name.append("AR/VR")
    ...
    # # print(dataframe)
    # dataframe.to_excel('./data_test.xls', index=False)
    # info('打印测试集')
    ...
    # print(dataframe)
    # with pd.ExcelWriter('./data_pre.xls') as writer:
    ...
    return (count0 / len(video_test) * 100), (count1 / len(iot_test) * 100), (count2 / len(car_test) * 100), \
           (count3 / len(AR_test) * 100), (count / len(test_x) * 100), \
           len(video_test), len(iot_test), len(car_test), len(AR_test), list_pre
...
if __name__ == '__main__':
    ...
    count4 = 0
    for i in range(n):
        count00, count11, count22, count33, count44, num_video, num_iot, num_car, num_AR, list_pre = train_and_predict()
        count0 = count0 + count00
        count1 = count1 + count11
        count2 = count2 + count22
        count3 = count3 + count33
    ...
    list_result = {'VIDEO个数': [num_video], 'IOT个数': [num_iot], '车载网个数': [num_car],
                   'AR/VR个数': [num_AR], '准确率': [str(round((count4 / n), 2)) + "%"]}
    dataframe1 = pd.DataFrame(list_pre)
    # print(dataframe1)
    dataframe2 = pd.DataFrame(list_result)
    # print(dataframe2)

    writer = pd.ExcelWriter('./data_pre.xls')
