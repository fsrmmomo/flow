...
from log_utils import  err

import shutil
from common_utils import save_json, debug, info, check_file, check_dir
...
class FilteredParser(Parser):
	...
	def get_pcap_name(self):
		pcap_name = str(self.fn.split("/")[-1])
		pcap_name = pcap_name.replace("\\", "_")
		pcap_name = pcap_name.rstrip(".pcap")
		return pcap_name
...
def generate_packets_info(flows: Dict[Tuple, List[Tuple]], dirname, pcap_name):
	# 每个只产生2000个流
	file_num = 0
	for specifier in flows.keys():
		if file_num > 200:
		...
		# 将每个包的大小和时间保存起来
		packets_info = []
		packets_info.append([sip, sport, dip, dport, proto])
		for i in range(len(timestamps)):
			packets_info.append([timestamps[i], pkt_size[i]])
		if len(packets_info) < 11:
			continue

		# 将目标解析后的数据保存到txt中
		txt_file = os.path.join(dirname, "{}_{}.txt".format(key, pcap_name))
		with open(txt_file, 'w') as f:
			for i in range(5):
				f.write(str(packets_info[0][i]) + ' ')
			f.write('\n')
			for i in range(1, len(packets_info)):
				f.write(str(packets_info[i][0]) + ' ')
				f.write(str(packets_info[i][1]))
				f.write('\n')


if __name__ == '__main__':
	pcaps_fn = { "iot":["E:/研究生/项目/21-01-19-OAI流类型识别/pcap文件/converted_iot","./tmp/cppkt/iot","iot"],
				 "video":["E:/研究生/项目/21-01-19-OAI流类型识别/pcap文件/video","./tmp/cppkt/video","video"],
				"voip":["E:/研究生/项目/21-01-19-OAI流类型识别/pcap文件/voip","./tmp/cppkt/voip","voip"],
				"AR": ["E:/研究生/项目/21-01-19-OAI流类型识别/pcap文件/AR", "./tmp/cppkt/AR", "AR"],
	}

	# pcaps_fn = {"iot": ["E:/研究生/项目/21-01-19-OAI流类型识别/pcap文件/converted_iot", "./tmp/dt/iot", "iot"],
	# 			"video": ["E:/研究生/项目/21-01-19-OAI流类型识别/pcap文件/video", "./tmp/dt/video", "video"],
	# 			"voip": ["E:/研究生/项目/21-01-19-OAI流类型识别/pcap文件/voip", "./tmp/dt/voip", "voip"],
	# 			"AR": ["E:/研究生/项目/21-01-19-OAI流类型识别/pcap文件/AR", "./tmp/dt/AR", "AR"],
	# 			}

	# pcaps_fn = {
	# 	"voip": ["E:/PyCharm/Project/testAccuracy/raw_data/voip_raw", "./tmp/dt/voip", "voip"],
	# }
	...
	for ftype in pcaps_fn.keys():
		...
		for file in files:
			...
			generate_packets_info(tcp, output_dir, str(pcap_name))
			generate_packets_info(udp, output_dir, str(pcap_name))
